...
set bit_file [file normalize [file join $repo_dir build lab9_top.bit]]
...
for {set attempt 1} {$attempt <= 2} {incr attempt} {
    puts "LAB9_PROGRAM_ATTEMPT=$attempt"
    if {[catch {program_hw_devices $dev} err]} {
        puts "LAB9_PROGRAM_ATTEMPT_${attempt}_ERROR=$err"
        after 1000
        refresh_hw_device $dev
    } else {
        set programmed 1
...
if {!$programmed} {
    error "lab_9 program failed after retries"
}

refresh_hw_device $dev
puts "LAB9_PROGRAMMED=$dev BITSTREAM=$bit_file"
close_hw_manager
